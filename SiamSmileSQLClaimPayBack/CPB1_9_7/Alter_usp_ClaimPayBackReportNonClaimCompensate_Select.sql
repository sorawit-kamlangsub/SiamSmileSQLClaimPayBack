...
-- Update date: 2026-07-14 Krekpon.D
-- Description:	เพิ่มข้อมูล AppId สถานศึกษา สถานะของกรมธรรม์ ของข้อมูลเคลมที่เป้น PA
-- =============================================
...
DECLARE @TmpClaimPayBack TABLE (
 ...
 WHERE   cpb.ClaimGroupTypeId = @ClaimGroupTypeId
    ...
    SELECT		icu.InsuranceCompany_Name													InsuranceCompany_Name
				...
				,IIF(@ClaimGroupTypeId IN (2,4) ,icu.AppID,NULL)							AppID
				,IIF(@ClaimGroupTypeId IN (2,4) ,icu.StatusPolicy,NULL)						StatusPolicy
				,IIF(@ClaimGroupTypeId IN (2,4) ,icu.SchoolName,NULL)						SchoolName
FROM @TmpClaimPayBack TmpCPB
	 LEFT JOIN(
			...
			SELECT DISTINCT pachg.Code							Code
				...
				,NULL											ClaimPaymentTypeName
				,NULL											ClaimPaymentDetailTypeName
				,cd.Application_id								AppID
				,_status.Detail									StatusPolicy
				,CONCAT(company.CompanyTitle,company.Detail)	SchoolName
			...
			LEFT JOIN SSSPA.dbo.DB_Customer customer
				ON cd.Application_id = customer.App_id
			LEFT JOIN SSSPA.dbo.MT_Company company
				ON company.Code = customer.School_id
			LEFT JOIN SSSPA.dbo.SM_Status _status
				ON _status.Code = customer.Status_id
